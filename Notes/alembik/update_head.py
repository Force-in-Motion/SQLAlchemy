...
# 1. Проверка истории:
# Alembic смотрит в таблицу alembic_version в БД (если её нет — создает) и узнаёт последнюю применённую версию (например, 1234abcd).

# 2. Поиск новых миграций:
# Сканирует папку versions/ и находит все файлы миграций, которые ещё не были применены (их ревизии не записаны в таблицу alembic_version)

# 3. Применение миграций:
# Выполняет код из функций upgrade() в каждом файле миграции по порядку.

# После каждой миграции обновляет таблицу alembic_version, записывая новую ревизию.
